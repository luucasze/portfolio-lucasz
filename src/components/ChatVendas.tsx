...
const chatSequence = [
    ...
    { id: 2, type: 'msg', from: 'eu', text: 'Oi. Fica 1.500 Reais. Entrego em 5 dias.', delay: 2240 },
    { id: 3, type: 'seen', from: 'eu', text: 'Visualizado', delay: 3360 },
    { id: 4, type: 'msg', from: 'cliente', text: 'Entendi... Pra ser sincero, não senti muita confiança pela forma que foi passado. Deixa pra próxima, obrigado.', delay: 4640 },
];

export const ChatVendas = () => {
    ...
    return (
        <div ref={containerRef} className="w-full h-full min-h-[400px] flex items-center justify-center">
            <div className="w-full bg-[#111] rounded-[2rem] border border-white/5 shadow-2xl overflow-hidden relative">
                {/* Barra de status do WhatsApp */}
                <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5 bg-[#0f0f0f]">
                    <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold text-primary">L</div>
                    <div className="flex-1">
                        <p className="text-white/80 text-sm font-medium">Lead Qualificado</p>
                ...
                {/* Feed de mensagens - Altura fixa sem scroll interno */}
                <div className="p-6 space-y-4 h-[520px] bg-[#0b0b0b]/50 backdrop-blur-sm relative transition-all duration-500 overflow-hidden">
                    ...
                    {chatSequence.map((item) => {
                        ...
                        if (item.type === 'seen') {
                            return (
                                <div
                                    key={item.id}
                                    className={`flex justify-end transition-all duration-700 delay-300 ${visible ? 'opacity-100' : 'opacity-0'}`}
                                >
                        ...
                        return (
                            <div
                                key={item.id}
                                className={`flex transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'} ${isMe ? 'justify-end' : 'justify-start'}`}
                            >
                                ...
                                >
                                    ...
                                    <div className="flex justify-end mt-1">
                                        <span className="text-[9px] text-white/40">20:41</span>
                                    </div>
                                </div>
                            </div>
                        );
                    ...
                    {/* Typing indicator */}
                    {started && visibleIds.length > 0 && !visibleIds.includes(4) && !visibleIds.includes(2) && visibleIds.includes(1) && (
                    ...
                    {started && visibleIds.includes(3) && !visibleIds.includes(4) && (
                ...
                <div className="px-4 py-3 bg-[#0f0f0f] flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-white/30">
                    ...
                    <div className="w-10 h-10 rounded-full bg-[#00a884] flex items-center justify-center text-white scale-90">
